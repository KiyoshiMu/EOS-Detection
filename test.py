...
class PictureWindow(tkinter.Canvas):
	...
	def show_image(self, path):
		...
		self.allready=self.create_image(self.winfo_screenwidth(),self.winfo_screenheight(),image=img, anchor='center', tag="bacl")
...
class SomethingWindow(Window):
    ...
    def show_image(self, path):
        ...
        panel1 = ttk.Label(self.contentframe, image=image)
        panel1.grid()
...
class GUI(ttk.Frame):
    ...
    def get_path(self, event=None):
        """Open img_dir"""
        path = filedialog.askopenfilename(defaultextension=".jpg",
