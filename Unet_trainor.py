...
def model_train(X_train, Y_train, X_test, Y_test, model_name=None, model_p=None):
    ...
    checkpointer = ModelCheckpoint(join('Unet_box',model_name+'.h5'), verbose=1, save_best_only=True)
    model.fit(X_train, Y_train, batch_size=18,
            verbose=1, epochs=100,
            validation_data=(X_test, Y_test),
            callbacks = [earlystopper, checkpointer,
                        TensorBoard(log_dir=join('Unet_box',model_name+'_log'), histogram_freq=0)])
    return model
